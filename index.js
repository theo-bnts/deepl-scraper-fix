...
module.exports = {
	translate: async (sentence, sourceLanguage = 'auto', targetLanguage) => {
		...
		let sentences = [];
		let _res = {};
		page.on('requestfinished', request => request.response().json().then(res => {
			if(!res['result']) return;
			sentences.push(...res['result']['translations'][0]['beams'].map(item => ({
	...
	getSupportedLanguages: async () => {
		...
		}
		{
			...
			let match;
			while(match = regExp.exec(data))
				res.targetLanguages.push(match[2] || `${match[1]}-${match[1].toUpperCase()}`);
			for(let i = res.targetLanguages.length - 1; i >= 0; i--){
				...
				if(
					...
					res.targetLanguages.find(_targetLanguage => {
						...
						return targetLanguageTags[0] === _targetLanguageTags[0] && _targetLanguageTags[1] !== _targetLanguageTags[0].toUpperCase();
					})
				){
